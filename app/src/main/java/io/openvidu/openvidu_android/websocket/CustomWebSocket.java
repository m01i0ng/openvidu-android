...
public class CustomWebSocket extends AsyncTask<SessionActivity, Void, Void> implements
    WebSocketListener {

  private final String TAG = "CustomWebSocketListener";
  private final int PING_MESSAGE_INTERVAL = 5;
  private final AtomicInteger RPC_ID = new AtomicInteger(0);
  private final AtomicInteger ID_PING = new AtomicInteger(-1);
  private final AtomicInteger ID_JOIN_ROOM = new AtomicInteger(-1);
  private final AtomicInteger ID_LEAVE_ROOM = new AtomicInteger(-1);
  private final AtomicInteger ID_PUBLISH_VIDEO = new AtomicInteger(-1);
  private final Map<Integer, Pair<String, String>> IDS_PREPARE_RECEIVE_VIDEO = new ConcurrentHashMap<>();
  private final Map<Integer, String> IDS_RECEIVE_VIDEO = new ConcurrentHashMap<>();
  private final Set<Integer> IDS_ON_ICE_CANDIDATE = Collections
  ...
  private void handleServerResponse(JSONObject json) throws
    ...
    } else if (rpcId == this.ID_JOIN_ROOM.get()) {
    ...
    } else if (rpcId == this.ID_LEAVE_ROOM.get()) {
      // Response to leaveRoom
      if (websocket.isOpen()) {
        websocket.disconnect();
      }
    } else if (rpcId == this.ID_PUBLISH_VIDEO.get()) {
      // Response to publishVideo
      LocalParticipant localParticipant = this.session.getLocalParticipant();
      SessionDescription remoteSdpAnswer = new SessionDescription(SessionDescription.Type.ANSWER,
          result.getString("sdpAnswer"));
      localParticipant.getPeerConnection()
          .setRemoteDescription(new CustomSdpObserver("publishVideo_setRemoteDescription"),
              remoteSdpAnswer);
    } else if (this.IDS_PREPARE_RECEIVE_VIDEO.containsKey(rpcId)) {
      // Response to prepareReceiveVideoFrom
      Pair<String, String> participantAndStream = IDS_PREPARE_RECEIVE_VIDEO.remove(rpcId);
      RemoteParticipant remoteParticipant = session
          .getRemoteParticipant(participantAndStream.first);
      String streamId = participantAndStream.second;
      SessionDescription remoteSdpOffer = new SessionDescription(SessionDescription.Type.OFFER,
          result.getString("sdpOffer"));
      remoteParticipant.getPeerConnection().setRemoteDescription(
          new CustomSdpObserver("prepareReceiveVideoFrom_setRemoteDescription") {
            @Override
    ...
    } else if (this.IDS_RECEIVE_VIDEO.containsKey(rpcId)) {
      // Response to receiveVideoFrom
      String id = IDS_RECEIVE_VIDEO.remove(rpcId);
      if ("kurento".equals(this.mediaServer)) {
        SessionDescription sessionDescription = new SessionDescription(
            SessionDescription.Type.ANSWER, result.getString("sdpAnswer"));
        session.getRemoteParticipant(id).getPeerConnection()
            .setRemoteDescription(new CustomSdpObserver("remoteSetRemoteDesc"), sessionDescription);
      }
    } else if (this.IDS_ON_ICE_CANDIDATE.contains(rpcId)) {
      // Response to onIceCandidate
      IDS_ON_ICE_CANDIDATE.remove(rpcId);
    } else {
  ...
  public void joinRoom() {
    ...
    joinRoomParams.put(JsonConstants.METADATA,
        "{\"clientData\": \"" + this.session.getLocalParticipant().getParticipantName() + "\"}");
    ...
    this.ID_JOIN_ROOM.set(this.sendJson(JsonConstants.JOINROOM_METHOD, joinRoomParams));
  }

  public void leaveRoom() {
    this.ID_LEAVE_ROOM.set(this.sendJson(JsonConstants.LEAVEROOM_METHOD));
  }
  ...
  public void publishVideo(SessionDescription sessionDescription) {
    ...
    this.ID_PUBLISH_VIDEO.set(this.sendJson(JsonConstants.PUBLISHVIDEO_METHOD, publishVideoParams));
  }
  ...
  public void prepareReceiveVideoFrom(RemoteParticipant remoteParticipant, String streamId) {
    ...
    this.IDS_PREPARE_RECEIVE_VIDEO
        .put(this.sendJson(JsonConstants.PREPARERECEIVEVIDEO_METHOD, prepareReceiveVideoFromParams),
            new Pair<>(remoteParticipant.getConnectionId(), streamId));
  }

  public void receiveVideoFrom(SessionDescription sessionDescription,
    ...
    this.IDS_RECEIVE_VIDEO
        .put(this.sendJson(JsonConstants.RECEIVEVIDEO_METHOD, receiveVideoFromParams),
  ...
  public void onIceCandidate(IceCandidate iceCandidate, String endpointName) {
    ...
    this.IDS_ON_ICE_CANDIDATE
        .add(this.sendJson(JsonConstants.ONICECANDIDATE_METHOD, onIceCandidateParams));
  }
  ...
  private void iceCandidateEvent(JSONObject params) throws JSONException {
    IceCandidate iceCandidate = new IceCandidate(params.getString("sdpMid"),
        params.getInt("sdpMLineIndex"), params.getString("candidate"));
    ...
    final Participant participant =
        isRemote ? session.getRemoteParticipant(connectionId) : session.getLocalParticipant();
  ...
  private void participantPublishedEvent(JSONObject params) throws
    ...
    final RemoteParticipant remoteParticipant = this.session
        .getRemoteParticipant(remoteParticipantId);
  ...
  private void participantLeftEvent(JSONObject params) throws JSONException {
    final RemoteParticipant remoteParticipant = this.session
        .removeRemoteParticipant(params.getString("connectionId"));
  ...
  private RemoteParticipant newRemoteParticipantAux(JSONObject participantJson)
    ...
    try {
      ...
      participantName = json.getString("clientData");
    } catch (JSONException e) {
      participantName = jsonStringified;
    }
    final RemoteParticipant remoteParticipant = new RemoteParticipant(connectionId, participantName,
        this.session);
  ...
  private void subscriptionInitiatedFromClient(RemoteParticipant remoteParticipant,
      String streamId) {
    MediaConstraints sdpConstraints = new MediaConstraints();
    ...
    remoteParticipant.getPeerConnection().createOffer(new CustomSdpObserver("remote offer sdp") {
      ...
      public void onCreateSuccess(SessionDescription sessionDescription) {
        ...
        remoteParticipant.getPeerConnection()
            .setLocalDescription(new CustomSdpObserver("remoteSetLocalDesc"), sessionDescription);
  ...
  private void subscriptionInitiatedFromServer(RemoteParticipant remoteParticipant,
      String streamId) {
    MediaConstraints sdpConstraints = new MediaConstraints();
    sdpConstraints.mandatory.add(new MediaConstraints.KeyValuePair("offerToReceiveAudio", "true"));
    sdpConstraints.mandatory.add(new MediaConstraints.KeyValuePair("offerToReceiveVideo", "true"));
  ...
  public void onDisconnected(WebSocket websocket, WebSocketFrame
    ...
    Log.e(TAG, "Disconnected " + serverCloseFrame.getCloseReason() + " " + clientCloseFrame
        .getCloseReason() + " " + closedByServer);
  }
