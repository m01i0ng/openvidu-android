...
public class Session {
  ...
  private final LinearLayout viewsContainer;
  private PeerConnectionFactory peerConnectionFactory;
  private CustomWebSocket webSocket;
  private final SessionActivity activity;

  public Session(String id, String token, LinearLayout viewsContainer, SessionActivity activity) {
    this.id = id;
    this.token = token;
    this.viewsContainer = viewsContainer;
    this.activity = activity;

    PeerConnectionFactory.InitializationOptions.Builder optionsBuilder = PeerConnectionFactory.InitializationOptions
  ...
  public PeerConnection createLocalPeerConnection() {
    ...
    PeerConnection peerConnection = peerConnectionFactory
        .createPeerConnection(rtcConfig, new CustomPeerConnectionObserver("local") {
          @Override
  ...
  public void createRemotePeerConnection(final String connectionId) {
    ...
    PeerConnection peerConnection = peerConnectionFactory
        .createPeerConnection(rtcConfig, new CustomPeerConnectionObserver("remotePeerCreation") {
          @Override
          public void onIceCandidate(IceCandidate iceCandidate) {
            super.onIceCandidate(iceCandidate);
    ...
    localParticipant.getPeerConnection().createOffer(new CustomSdpObserver("createOffer") {
      ...
      public void onCreateSuccess(SessionDescription sessionDescription) {
        ...
        localParticipant.getPeerConnection()
            .setLocalDescription(new CustomSdpObserver("createOffer_setLocalDescription"),
                sessionDescription);
  ...
  public void createAnswerForSubscribing(RemoteParticipant remoteParticipant, String streamId,
      MediaConstraints constraints) {
    remoteParticipant.getPeerConnection()
        .createAnswer(new CustomSdpObserver("createAnswerSubscribing") {
          @Override
          public void onCreateSuccess(SessionDescription sessionDescription) {
            super.onCreateSuccess(sessionDescription);
            Log.i("createAnswer SUCCESS", sessionDescription.toString());
            remoteParticipant.getPeerConnection().setLocalDescription(
                new CustomSdpObserver("createAnswerSubscribing_setLocalDescription") {
                  @Override
  ...
  public void leaveSession() {
    ...
    this.activity.runOnUiThread(() -> {
      for (RemoteParticipant remoteParticipant : remoteParticipants.values()) {
        ...
        viewsContainer.removeView(remoteParticipant.getView());
      }
    });
    AsyncTask.execute(() -> {
  ...
  public void removeView(View view) {
    this.viewsContainer.removeView(view);
  }
